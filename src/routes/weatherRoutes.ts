...
router.get("/search", async (req, res) => {
  try {
    ...
    const cities = geoResponse.data.results.map((result: any) => ({
      city: result.name,
...
router.get("/history", async (req, res) => {
  try {
    ...
    const historyResponse = await axios.get(
      `https://archive-api.open-meteo.com/v1/archive?latitude=${latitude}&longitude=${longitude}&start_date=${startDate.toISOString().split('T')[0]}&end_date=${endDate.toISOString().split('T')[0]}&daily=temperature_2m_mean,precipitation_sum&timezone=auto`
    );

    const historyData = historyResponse.data.daily.time.map((date: string, i: number) => ({
      date,
...
router.get("/hourly", async (req, res) => {
  try {
    ...
    const hourlyResponse = await axios.get(
      `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=temperature_2m,relative_humidity_2m,precipitation_probability,weather_code,wind_speed_10m&timezone=auto&forecast_days=1`
    );

    const hourlyData = {
      city: name,
      country,
      hourly: hourlyResponse.data.hourly.time.map((time: string, i: number) => ({
        time,
